#!/usr/bin/env python3
"""gemini31_pro.py — Google Gemini 3.1 Pro Preview, direct API, one-call spike.
...
`gemini36_flash.py` both load at runtime. Pro-tier counterpart to
`gemini36_flash.py`; both files are intentionally near-identical (the
harness's other backends — whisperx/gemini-3-1-flash-lite/assemblyai — are
also fully standalone per eval/lyrics/README.md's "Adding a new backend"
convention, so this duplicates request/parse logic rather than factoring a
shared `_gemini_common.py` helper module).
...
Usage:
    python eval/lyrics/backends/gemini31_pro.py \\
...
logger = logging.getLogger("lyrics_eval.gemini31_pro")

BACKEND_ID = "gemini31-pro"
BACKEND_REVISION = 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info(
        "gemini31_pro starting: wav=%s out=%s model=%s", args.wav, args.out, MODEL_SLUG
    )
    ...
    logger.info("gemini31_pro call complete: elapsed_s=%.1f", elapsed)
    ...
    logger.info(
        "gemini31_pro done: lines=%d out=%s elapsed_s=%.1f",
        len(lines),
        args.out,
        elapsed,
    )
