...
"#
        ...
        // CLIProxyAPI 7.x dropped the top-level `claude-oauth` block that 6.9.x
        // used to force `cloak: never`; cloak is now on by default and disabled
        // only via `disable-claude-cloak-mode`. We deliberately keep v7's
        // default-on cloak (it makes the OAuth requests look like Claude Code,
        // which is what Anthropic's subscription backend expects) — verified on
        // win-resolume that completions succeed with it (#145). Against 7.3.1
        // this removal is a no-op: the running proxy already ignored the stale
        // block, so the pre-cleanup live completions were already under
        // default-on cloak. A rollback to a 6.9.x build would parse the
        // block-less config fine but fall back to THAT build's default cloak
        // rather than the old explicit `never`, so the rollback procedure in
        // scripts/cliproxy/README.md re-checks a completion after restoring.
