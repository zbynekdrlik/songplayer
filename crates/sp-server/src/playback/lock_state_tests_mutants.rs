...
//! `push` carries the cumulative `seq` as its 2nd arg (#168 round 6).
//!
//! (The pinned-skip mutant `evict` `> -> <` is NOT targeted — it makes the
...
/// A lone `seq` decrease (all other counters unchanged) is a re-anchor reset:
/// the ring is cleared and restarted, so `len == 1`. Pins the `seq < back.seq`
/// comparison + its `||` arm added in #168 round 6.
#[test]
fn a_lone_seq_decrease_triggers_a_reset() {
    let mut w = EventWindow::new();
    w.push(0, 5, 5, 5, 5);
    w.push(U, 3, 5, 5, 5); // seq 5 -> 3, others unchanged
    assert_eq!(
        w.len(),
        1,
...
/// A lone `late` decrease (seq/repeats/resyncs unchanged) is a re-anchor reset,
/// so `len == 1`. Pins the `late < back.late` arm's `||` (a `|| -> &&` there
/// makes the compound condition false → no reset → `len == 2`).
#[test]
fn a_lone_late_decrease_triggers_a_reset() {
    let mut w = EventWindow::new();
    w.push(0, 10, 5, 5, 5);
    w.push(U, 11, 3, 5, 5); // seq up, late 5 -> 3, others unchanged
    assert_eq!(
        w.len(),
        1,
        "reset clears the ring, keeping only the new sample"
...
/// A lone `repeats` decrease is a re-anchor reset, so `len == 1`. Pins the
/// `repeats < back.repeats` arm's `||`.
#[test]
fn a_lone_repeats_decrease_triggers_a_reset() {
    let mut w = EventWindow::new();
    w.push(0, 10, 5, 5, 5);
    w.push(U, 11, 5, 3, 5); // seq up, repeats 5 -> 3, others unchanged
    assert_eq!(
        w.len(),
        1,
        "reset clears the ring, keeping only the new sample"
...
///
/// Kills the cutoff `- -> /`: `75s / 60s` (integer, in 100-ns units) collapses
/// the cutoff to ~0, so nothing is evicted and `len == 3`.
#[test]
...
///
/// Kills `> -> <`: `ring.len() < 1` is always false right after a push
...
///
/// Kills `< -> <=`: `15s <= 15s` would evict the boundary sample → `len == 2`.
#[test]
...
/// `len == 2`.
///
/// Kills `< -> ==`: `10s == 20s` is false → the strictly-older sample is
/// kept → `len == 3`.
#[test]
