//! The paced NDI submit side (#168 output-side split): the emit→submit handoff
//! and the submit consumer, cross-platform so Linux tests drive them over
//! `MockNdiBackend` (moved out of the Windows-only `pipeline_paced_submit.rs`).
//!
...
//!
//! A dedicated submit thread owns the [`FrameSubmitter`] and drains the bounded
//! [`SharedHandoff`]. The emit thread only stamps a frame and hands it over in
//! ~µs, so it stays on the grid; the submit thread absorbs the
//! `send_video_async` stalls.
//!
...
//! this file is the `Mutex`/`Condvar` + thread plumbing.
...
use crate::playback::submit_handoff::{
    HandoffOutcome, PacedSubmitStats, SubmitCounters, SubmitJob, SubmitQueue,
