...
use chrono::Utc;
use sp_core::playback::{KaraokeMode, PlaybackMode, PlaybackState as WsPlaybackState};
...
impl PlaybackEngine {
    ...
    /// #14: apply a new karaoke mode + vocal gain. Updates the process-global
    /// live control, persists both to settings (restored on restart),
    /// broadcasts the new state, and — when the MODE changed — reloads every
    /// playing pipeline at its current position so the wall reflects it
    /// immediately (the vocal-gain slider is already live per audio chunk, so a
    /// gain-only change needs no reload).
    #[cfg_attr(test, mutants::skip)]
    pub async fn set_karaoke(&mut self, mode: KaraokeMode, vocal_gain: f32) {
        ...
        // Mode change swaps which files the decoder opens → reload each playing
