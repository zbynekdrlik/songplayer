...
// ── Phase 2.65: absorb_leading_unmatched ──────────────────────────────────────

#[test]
fn absorb_leading_unmatched_claims_misheard_lead_words() {
    // id=21 2:12 regression: whisperx mistranscribed "shadow" as "shed on"
    // (asr[81-82]); LCS could not match → unmatched; line "shadow me…"
    // started at "me" (asr[83]) instead of singer's first audible sound
    // at "shadow" (asr[81]). Phase 2.65 walks back through unmatched
    // ASR words and attaches them to the next emit so its natural start
    // moves back to the singer's true line-start.
    let asr_track = asr(vec![
        ...
        make_word("shed", 131741, 132541), // misheard "shadow"
        make_word("on", 132781, 132941),   // misheard part of "shadow"
    ...
    assert_eq!(
        ...
        vec![5, 6, 7, 8, 9],
        "unmatched 'shed', 'on' (misheard 'shadow') must attach to 'shadow me…'"
    );
    assert_eq!(
