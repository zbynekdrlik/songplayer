...
//!   1. Restamps any row ABOVE the current `LYRICS_PIPELINE_VERSION` down to
//!      19 (un-anomalies rows produced by a now-reverted code path that
//!      bumped the constant without approval — an "anomaly" is any row
//!      stamped higher than the constant the worker actually produces).
...
pub async fn reprocess_catalog_with_new_gate(State(state): State<AppState>) -> impl IntoResponse {
    ...
    // Step 1: restamp the anomaly. An "anomaly" is any row stamped ABOVE the
    // current LYRICS_PIPELINE_VERSION — a row a now-reverted code path
    // produced by bumping the constant without approval. Bound to the
    // constant (not a literal) so the threshold tracks legitimate bumps:
    // once #143 moved the constant 20 → 21, a freshly-processed v21 row is
    // the current version, NOT an anomaly, and must never be restamped back
    // to 19; only rows above the constant are.
...
mod tests {
    ...
    async fn setup_pool_with_videos() -> SqlitePool {
        ...
        .await
        .unwrap();
        // Row 1: anomaly (LYRICS_PIPELINE_VERSION + 1 — above the current
        // constant) — should be restamped to 19, then queued.
        sqlx::query(&format!(
            "INSERT INTO videos (playlist_id, youtube_id, title, lyrics_source, lyrics_pipeline_version) \
             VALUES (1, 'aaa', 't1', 'yt_subs', {})",
            LYRICS_PIPELINE_VERSION + 1
        ))
        .execute(&pool)
        .await
        .unwrap();
        // Row 2: v15 (stale, < the constant), ordinary lyrics — should be queued.
        sqlx::query(
            "INSERT INTO videos (playlist_id, youtube_id, title, lyrics_source, lyrics_pipeline_version) \
        ...
        .await
        .unwrap();
        // Row 5: current version (LYRICS_PIPELINE_VERSION) — must NOT be
        // queued (already at current; not < the constant).
        sqlx::query(&format!(
            "INSERT INTO videos (playlist_id, youtube_id, title, lyrics_source, lyrics_pipeline_version) \
             VALUES (1, 'eee', 't5', 'yt_subs', {})",
    ...
    async fn endpoint_restamps_only_rows_above_current_version() {
        ...
        assert_eq!(
            result.restamped, 1,
            "only the anomaly row (above the current constant) must be restamped"
        );
    ...
    async fn endpoint_queues_eligible_rows_and_skips_sentinels() {
        let pool = setup_pool_with_videos().await;
        let result = run_endpoint_sql(&pool).await;
        // After restamp: the anomaly row (was LYRICS_PIPELINE_VERSION + 1) is
        // now v19, and v15 + v18 (unsupported_source) are all below the
        // constant. The queue UPDATE excludes asr_gap + unsupported_source,
        // and the v5 "current" row (== the constant) is not `<` it. Eligible:
        // the restamped v19 yt_subs row + v15 lrclib = 2.
    ...
    async fn endpoint_is_idempotent_on_second_call() {
        ...
        //   - Restamp: WHERE lyrics_pipeline_version > LYRICS_PIPELINE_VERSION →
        //     no rows match after the first call moved the anomaly down to 19.
        //   - Cleanup: WHERE lyrics_manual_priority = 1 AND lyrics_source IN
        //     (terminal sentinels) → no rows match after the first call cleared
        //     them.
        //   - Queue: WHERE lyrics_pipeline_version < LYRICS_PIPELINE_VERSION AND
        //     lyrics_manual_priority = 0 → no rows match after the first call
        //     set every eligible row's manual_priority to 1.
    ...
    async fn endpoint_router_returns_correct_counts_and_clears_dangling() {
        ...
        // Seed a mix (versions relative to LYRICS_PIPELINE_VERSION):
        //   - r-v21: anomaly at the constant + 1 (restamped → 19, then queued)
        //   - r-v15: v15, stale lrclib row (will be queued)
        //   - r-ns:  no_source row at the constant WITH manual_priority=1
        //            (dangling flag cleared; NOT re-queued — not < the constant)
        //   - r-asr: asr_gap row at the constant WITH manual_priority=1
        //            (must NOT be cleared; parked by the skip-list)
        sqlx::query(
            "INSERT INTO playlists (id, name, youtube_url, is_active) VALUES (1, 'p', 'u', 1)",
        )
        .execute(&state.pool)
        ...
        sqlx::query(&format!(
            "INSERT INTO videos (playlist_id, youtube_id, title, lyrics_source, \
                                 lyrics_pipeline_version, lyrics_manual_priority) VALUES \
             (1, 'r-v21', 't', 'yt_subs', {anomaly}, 0), \
             (1, 'r-v15', 't', 'lrclib', 15, 0), \
             (1, 'r-ns',  't', 'no_source', {current}, 1), \
             (1, 'r-asr', 't', 'asr_gap', {current}, 1)",
            anomaly = LYRICS_PIPELINE_VERSION + 1,
        ...
        // Verify post-state in the DB (ORDER BY youtube_id → r-asr, r-ns,
        // r-v15, r-v21):
        //   - r-asr → still at the constant, manual_priority 1 (parked)
        //   - r-ns  → still at the constant, manual_priority 0 (cleared)
        //   - r-v15 → manual_priority 1 (queued)
        //   - r-v21 → restamped to version 19, manual_priority 1 (queued)
        ...
        assert_eq!(
            rows,
            vec![
                ("r-asr".into(), LYRICS_PIPELINE_VERSION as i64, 1),
