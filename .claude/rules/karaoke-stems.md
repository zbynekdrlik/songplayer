...
**The deadline is the decoder's `audio_lead_ms`, so the fader latency depends on
the path (#148 v4):**

| path | deadline | fader latency |
|---|---|---|
...
- **Bounded.** The lead is a FIXED cushion. The G5 gate still stops it growing,
  so the latency stays ≤ lead + one chunk for the whole song.
