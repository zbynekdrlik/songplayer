...
- **SongPlayer side (`sp_min_working_set_mb`, default 3072 MiB).**
  - `process_start::apply_min_working_set` runs once after the DB is ready. It
    enables `SeIncreaseWorkingSetPrivilege`, then calls `SetProcessWorkingSetSizeEx`
    with `QUOTA_LIMITS_HARDWS_MIN_ENABLE | QUOTA_LIMITS_HARDWS_MAX_DISABLE`.
